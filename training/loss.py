    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                # sparsity loss
                if self.sparse_weight > 0:
                    loss_Gsparse = self.sparse_weight * gen_style.abs().mean()
                else:
                    loss_Gsparse = torch.tensor(0.0, device=self.device)
                if self.mask_sparse_weight > 0:
                    loss_Gsparse += self.mask_sparse_weight * self.G_mapping.mask.abs().mean()
                training_stats.report('Loss/G/sparse', loss_Gsparse)
                ...
                loss_Gmain = loss_Ggan + loss_Gsparse
        ...
        if do_Gpl:
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                gen_img, gen_ws, gen_content, gen_style = self.run_G(gen_z[:batch_size], gen_c[:batch_size], sync=sync)
